...
def balanced(m):
    ...
    if is_weight(m):
        ...
        torque_left=lenghth(left(m))*total_weight(left_end)
        torque_right=lenghth(right(m))*total_weight(right_end)
        return balance(left_end) and balance(right_end) and torque_left==torque_right
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint(amount, password_attemp):
        if password_attemp==new_password:
            return withdraw(amount, old_password)
        return withdraw(amount, password_attemp)
    return joint  
